...
public class TS_GVMFixInputUtils {
    ...
    public static TGS_UnionExcuse<Rectangle> screen_size(TS_ThreadSyncTrigger kill, Duration maxDuration) {
        ...
        if (!TS_FileUtils.isExistFile(jar)) {
            return TGS_UnionExcuse.ofExcuse(TS_GVMFixInputUtils.class.getSimpleName(), "screen_size", "File not found: " + jar.toString());
        }
        ...
        return TGS_UnionExcuse.of(new Rectangle(lstInt.get(0), lstInt.get(1), lstInt.get(2), lstInt.get(3)));
    }

    public static TGS_UnionExcuse<Float> screen_scale(TS_ThreadSyncTrigger kill, Duration maxDuration) {
        var jar = TS_PathUtils.getPathCurrent_nio().getParent().resolve(Main.class.getPackageName()).resolve(Main.class.getPackageName() + ".jar").toAbsolutePath();
        if (!TS_FileUtils.isExistFile(jar)) {
            return TGS_UnionExcuse.ofExcuse(TS_GVMFixInputUtils.class.getSimpleName(), "screen_scale", "File not found: " + jar.toString());
        }
        ...
        return TGS_UnionExcuse.of(flt);
    }

    public static TGS_UnionExcuse<BufferedImage> screen_shot(TS_ThreadSyncTrigger kill, Duration maxDuration) {
        return screen_shot(kill, maxDuration, null);
    }

    public static TGS_UnionExcuse<BufferedImage> screen_shot(TS_ThreadSyncTrigger kill, Duration maxDuration, Float scale) {
        var jar = TS_PathUtils.getPathCurrent_nio().getParent().resolve(Main.class.getPackageName()).resolve(Main.class.getPackageName() + ".jar").toAbsolutePath();
        if (!TS_FileUtils.isExistFile(jar)) {
            return TGS_UnionExcuse.ofExcuse(TS_GVMFixInputUtils.class.getSimpleName(), "screen_shot", "File not found: " + jar.toString());
        }
        ...
        if (img == null) {
            return TGS_UnionExcuse.ofExcuse(TS_GVMFixInputUtils.class.getSimpleName(), "screen_shot", "returned not path-able: " + value);
        }
        return TGS_UnionExcuse.of(img);
    }
}
