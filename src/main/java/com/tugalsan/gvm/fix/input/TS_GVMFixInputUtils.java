    ...
    public static TGS_Optional<Rectangle> size(TS_ThreadSyncTrigger kill, Duration maxDuration) {
        ...
        if (!TS_FileUtils.isExistFile(fileSize)) {
            ...
            if (!TS_FileUtils.isExistFile(fileConsoleApp)) {
                return TGS_Optional.ofEmpty("ERROR @", TS_GVMFixInputUtils.class.getSimpleName(), "size", "File not found", fileConsoleApp.toString());
            }
            ...
            if (await.exceptionIfFailed.isPresent()) {
                return TGS_Optional.ofEmpty("ERROR @", TS_GVMFixInputUtils.class.getSimpleName(), "size", "Screensize returned empty", await.exceptionIfFailed.get().toString(), fileConsoleApp.toString());
            }
            var strSize = await.resultIfSuccessful.get();
            if (TGS_StringUtils.isNullOrEmpty(strSize)) {
                return TGS_Optional.ofEmpty("ERROR @", TS_GVMFixInputUtils.class.getSimpleName(), "size", "Screensize returned empty", fileConsoleApp.toString());
            }
            TS_FileTxtUtils.toFile(strSize, fileSize, false);
        }
        if (!TS_FileUtils.isExistFile(fileSize)) {
            return TGS_Optional.ofEmpty("ERROR @", TS_GVMFixInputUtils.class.getSimpleName(), "size", "Cannot write fileSize", fileSize.toString());
        }
        var strSize = TS_FileTxtUtils.toString(fileSize);
        if (TGS_StringUtils.isNullOrEmpty(strSize)) {
            return TGS_Optional.ofEmpty("ERROR @", TS_GVMFixInputUtils.class.getSimpleName(), "size", "File content is empty", fileSize.toString());
        }
        var valuesStr = TS_StringUtils.toList_spc(strSize);
        if (valuesStr.size() != 4) {
            return TGS_Optional.ofEmpty("ERROR @", TS_GVMFixInputUtils.class.getSimpleName(), "size", "File content values count is not 4", fileSize.toString());
        }
        var valuesInt = TGS_StreamUtils.toLst(valuesStr.stream().map(s -> TGS_CastUtils.toInteger(s)).filter(i -> i != null));
        if (valuesInt.size() != 4) {
            return TGS_Optional.ofEmpty("ERROR @", TS_GVMFixInputUtils.class.getSimpleName(), "size", "File content values that can be converted to integer count is not 4", fileSize.toString());
        }
